...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(const std::function<double(Position)> arguments) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)> arguments) const override {
                ...
                case Add:
                    tmpRes = lhs_->Evaluate(arguments) + rhs_->Evaluate(arguments);
                    break; 
                case Subtract:
                    tmpRes = lhs_->Evaluate(arguments) - rhs_->Evaluate(arguments);
                    break;
                ...
                /*Не только чтобы определять деление на 0*/
                if (!std::isfinite(tmpRes)) {                                       //isfinite - "isfinite проверяет, является ли аргумент конечным числом, то есть проверяет, что аргумент не плюс/минус бесконечность и не является нечисловым значением (не NAN)."
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)> arguments) const override {
                ...
                if (type_ == UnaryMinus) {
                    ...
                    return -1 * operand_->Evaluate(arguments);

                }
                else { 
                    return operand_->Evaluate(arguments);
                }
        ...
        public:
            explicit CellExpr(const Position* cell)                     //конструктор
                : cell_(cell) {
            }
            ...
            double Evaluate(std::function<double(Position)> arguments) const override {
                return arguments(*cell_);
            }
        ...
        public:
            ...
            void exitCell(FormulaParser::CellContext* ctx) override {
                auto tmpStr = ctx->CELL()->getSymbol()->getText();
                auto value = Position::FromString(tmpStr);

                if (!value.IsValid()) {
                    throw FormulaException("Position not valid: " + tmpStr);
                }

                effCells_.push_front(value);
                auto node = std::make_unique<CellExpr>(&effCells_.front());
                args_.push_back(std::move(node));
            }
        ...
        private:
            ...
            std::forward_list<Position> effCells_;
        };
...
double FormulaAST::Execute(const std::function<double(Position)> arguments) const {
    return root_expr_->Evaluate(arguments);
}

FormulaAST::FormulaAST(std::unique_ptr<ASTImpl::Expr> root_expr, std::forward_list<Position> inputCells)
    : root_expr_(std::move(root_expr))
    , efficientCellStorage_(std::move(inputCells))
{
    efficientCellStorage_.sort();  //Иначе придётся сортировать в куче других мест
...
void FormulaAST::PrintCells(std::ostream& out) const {
    for (auto cell : efficientCellStorage_) {
        out << cell.ToString() << ' ';
    }
}